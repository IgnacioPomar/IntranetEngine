...
class UUIDv7
{
	...
	public static function inverseB64 ($uuid): string
	{
		...
		// Convierte los primeros 8 bytes al componente de bits más significativo
		for($i = 0; $i < 8; $i ++)
		{
			$mostSigBits = ($mostSigBits << 8) | (ord ($uuidBytes [$i]) & 0xFF);
		}

		// Convierte los siguientes 8 bytes al componente de bits menos significativo
		for($i = 8; $i < 16; $i ++)
		{
			$leastSigBits = ($leastSigBits << 8) | (ord ($uuidBytes [$i]) & 0xFF);
		}

		// Extrae el tiempo del componente de bits más significativo
